...
/**
 * Metal RestManagementToolbar component.
...
class RestManagementToolbar extends ClayComponent {
	...
	_handleOrderItemClicked(event) {
		return !this.emit({
			data: event.data,
			name: 'orderItemClicked',
			originalEvent: event,
		});
	}
	_handleOnFiltersSubmit(event) {
		return !this.emit({
			name: 'onFiltersSubmited',
			originalEvent: event,
		});
	}
	_handleOnFormSubmit(event) {
		let searchValue = event.target[0].value;
		let searchActionUrl = event.target.action;
		let searchFormMethod = event.target.method;
		return !this.emit({
			data: {
...
Soy.register(RestManagementToolbar, templates);

export {RestManagementToolbar};
export default RestManagementToolbar;